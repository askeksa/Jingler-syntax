...
**Version**: 2.0.1 | **Publisher**: askeksa | **Engine**: VS Code ^1.50.0 | **Tests**: 312 passing, 0 lint errors
...
| File | Lines | Purpose |
|---|---|---|
| `extension.ts` | 50 | Entry point — registers all providers, wires diagnostics to document events, logging |
| `document_symbols.ts` | 192 | Tokenizer + parser orchestration, symbol extraction, `ZingDocument` class |
| `definitions.ts` | 210 | Shared symbol lookup (`symbolAt`, `findSymbolInDocument`, `findSymbolInIncludes`), `LookupResult`, `DefinitionProvider` |
| `hover.ts` | 304 | `HoverProvider`, `BUILT_INS` table (33 built-ins with signatures, descriptions, arg counts, context, memberKind) |
| `diagnostics.ts` | 878 | `computeDiagnostics`, `DiagnosticCollection`, 8 diagnostic categories, include walker, diagnostic logging |
...
| `parser.ts` | 684 | Recursive descent parser, `MidiParserContext` impl, error tracking, recovery, context defaulting, precedence climbing |
...
| `ast.ts` | 230 | All AST node types, `Position`, `ContextKind`, `MemberKind`, etc. |
| `logging.ts` | 3 | `channel` — VS Code output channel ("Zing") for diagnostic/logging output |
...
| File | Lines | Purpose |
|---|---|---|
| `syntaxes/zing.tmLanguage.json` | 96 | TextMate grammar for fallback syntax highlighting (comments, keywords, numbers, operators) |
| `src/test/vscode.ts` | 250 | VS Code API mock for running tests natively without extension host |
...
- `LookupResult` — member?, parameter? (returned by `findSymbolInIncludes` in `definitions.ts`)
...
- `parseBinaryLeft(left, ops, parseRight)` takes a `parseRight` callback for correct precedence (e.g. `parseAnd` calls `parseBinaryLeft(parseCompare, ["And"], () => parseCompare())`)
...
- **Forward refs allowed by default**: only flagged when outside `cell`/`delay`/`dyndelay` calls or for-loop scope. Cross-member forward refs are allowed (members can reference each other in any order).
...
- **Output/input passthrough**: outputs sharing a name with an input are valid (e.g. `function lowp(x) → x`). Outputs are checked against their own map, not the shared names map, so body assignments to output names don't falsely collide with the output declaration. Matches real compiler `names.rs` which never inserts outputs into the variables map.
...
- **Forward refs to members allowed**: only local variable forward refs within a member are flagged; cross-member references resolve freely (real compiler: `lookup_member` has no forward check)
- **Logging**: `src/logging.ts` exposes a VS Code output channel ("Zing") for diagnostic timing and counts
- **TextMate grammar**: `syntaxes/zing.tmLanguage.json` provides fallback syntax highlighting (comment coloring, keywords, numbers, operators)
- **Hover**: no backticks on names, `formatPattern` returns `undefined` for empty patterns
...
312 Mocha tests run natively via `node out/test/runMocha.js` — uses `src/test/vscode.ts` mock instead of `@vscode/test-electron`, so tests run outside the VS Code extension host. `runMocha.ts` intercepts `require('vscode')` to load the mock.

| File | Lines | Tests | Coverage |
|---|---|---|---|
| `diagnostics.test.ts` | 1009 | 301 | Parser errors, unresolved/resolved variables, forward refs, built-ins, ForExpr, merge/conditional/buffer/tuple/unary expressions, method calls, include resolution, severity, ranges, duplicate detection, context errors, argument count errors, call context errors (13 rules), bytecode emitter errors, relatedInformation, output/input passthrough |
| `hover.test.ts` | 624 | 42+ | Members, parameters, locals, built-ins (all 30), expressions (all kinds), edge cases, included-file hover |
...
| `syntax_highlighting.test.ts` | 224 | ~24 | Tokenizer defaults, semantic tokens, legend, operator precedence (9 tests) |
| `document_symbols.test.ts` | 75 | 10 | Symbol extraction, include directives, comment skipping |
| `vscode.ts` | 250 | — | Mock of VS Code API (Position, Range, Diagnostic, TextDocument, Uri, workspace, window, etc.) |
...
```
just init          # npm install + @types/mocha + global vsce/ovsx
npm run compile    # TypeScript → out/
npm run lint       # ESLint on src/**/*.ts
npm test           # run Mocha tests natively (vscode mock, no extension host)
just package       # compile + vsce package
